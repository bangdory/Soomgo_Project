...
<head>
    ...
    <style>
        ...
        .receivedAllRequests, .receivedRequests {
            flex: 1 1 calc(50% - 20px);
            box-sizing: border-box;
        }

        .answeredRequests, .receivedRequests {
            /* Flexbox를 사용하여 자식 요소를 수평으로 배치합니다. */
            flex: 1 1 calc(50% - 20px); /* 각 요소가 50% 너비를 가지며, 간격을 고려합니다. */
            box-sizing: border-box; /* 테두리와 여백을 포함한 박스 모델을 사용합니다. */
        }
        ...
        .answeredRequestDetail, .receivedRequestDetail {
...
<div class="requestContainer">
    <div class="receivedAllRequests">
        ...
        <div class="answeredRequests">
            ...
            <%--        <c:if test="${answeredList[0].ignored == 0 && receivedList[0].deleted == 0}">--%>
            <%--            <h3>견적을 보낸 요청서</h3>--%>
            <%--        </c:if>--%>
            <c:forEach var="answered" items="${answeredList}">
        ...
        <div class="receivedRequests">
            ...
            <c:forEach var="received" items="${receivedList}">
                <c:if test="${received.expertAlreadyAnswered == 0 && received.ignored == 0}">
                    <div class="receivedRequestDetail">
                        <div>
                                ${received.userName} 님의 ${received.typeName} 요청서
                        </div>
                        <button class="readDetail"
                                onclick="document.getElementById('readDetailsFromClient').showModal()">
                            자세히 보기
                        </button>
                        <dialog id="readDetailsFromClient">
                            ...
                            <button class="btn-deleted"
                                    onclick="document.getElementById('readDetailsFromClient').close()">
                                닫기
                            </button>
                            ...
                            <div class="receivedRequest">
                                ...
                                <form action="/request/ignoreReceivedRequest/${received.id}" method="post">
                                    ...
                                    <button name="ignoreReceived" type="submit" class="btn btn-warning">견적 지우기(구현전)
                                    </button>
                                </form>
                                </c:if>
                        </dialog>
